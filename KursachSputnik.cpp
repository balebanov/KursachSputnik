...
int main()
{
	...
	// 1. реализовать функцию транспонирования
	// 2. разобраться с перемножением транспонированной матрицы на себя
	// 3. реализовать алгоритм нахождения обратной матрицы 4х4
	// если это все сделать, то дальше по хуйне
...
void RungeKutt(double t_i, double t_mdv, double h, double s[6], double arg[6], double f[6], GLOEphemeris R) {
	...
	for (double ti = t_i; ti <= t_mdv; ti = ti + h) {
		...
		for (int i = 0; i < 6; i++) arg[i] = s[i] + 0.5 * k2[i];
		reCalc(arg, R, f);
		for (int i = 0; i < 6; i++) k3[i] = h * f[i];
		for (int i = 0; i < 6; i++) arg[i] = s[i] + k3[i];
		reCalc(arg, R, f);
